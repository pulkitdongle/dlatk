...
class SqlWrapper(object):
    ...
    def connect(self):
        if self.db_type == "mysql":
            self.sqlObject = MySqlWrapper(self.corpdb, self.sql_host, self.encoding)
            return self.sqlObject.get_db_connection()
        if self.db_type == "sqlite":
            # instantiate SqliteWrapper creating the connection with database and return its object
            pass

    def disable_table_keys(self, featureTableName):
        self.sqlObject.disable_table_keys(featureTableName)

    def enable_table_keys(self, featureTableName):
        self.sqlObject.enable_table_keys(featureTableName)

    def execute_get_list(self, usql):
        return self.sqlObject.execute_get_list(usql)


class MySqlWrapper(SqlWrapper):
...
class SqliteWrapper(SqlWrapper):
